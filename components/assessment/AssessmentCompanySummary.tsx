...
export default function AssessmentCompanySummary({ company }: AssessmentCompanySummaryProps) {
  const hasOptionalInfo = company.legalEntity || company.location || company.contactPerson || company.stakeholders.length > 0 || company.certifications.length > 0;

  return (
    <Box>
      <Group mb={8} gap={8}>
        <IconBuilding size={20} color="var(--mantine-color-blue-7)" />
        <Title order={5} fw={600}>Company Information</Title>
      </Group>
      ...
      {hasOptionalInfo && (
        <Box ml="md">
          ...
          {company.stakeholders.length > 0 && (
            <Box mb={4}>
              <Text size="sm" fw={500} mb={2}>Stakeholders:</Text>
              <Group gap={4}>
          ...
          {company.certifications.length > 0 && (
            <Box>
              <Text size="sm" fw={500} mb={2}>Certifications:</Text>
              <Group gap={4}>
